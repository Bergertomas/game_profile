# Game Profile — Master Product & Build Plan v0.4
...
- freeze strongly unknowable areas rather than manufacture precision.
...
- IGDB is technically strong and free for non-commercial use; commercial use requires partnership.
- RAWG provides rich metadata and has commercial-use conditions/thresholds plus attribution requirements.

Therefore:
1. Build an adapter interface.
2. Do not couple schema to one provider.
3. For beta, manual metadata entry is acceptable.
4. If public/commercial ambitions grow, explicitly resolve provider licensing before relying on their images/data.
