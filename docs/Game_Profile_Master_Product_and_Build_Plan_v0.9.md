...
two measured D1 research attempts are immutable evidence: attempt 1 was refused
before freeze, and attempt 2 ended `failed_api` after 300095 ms with null
returned model/response ID/usage, no response body and no attempt directory, so
provider processing and spend cannot be established. No real corpus, accepted
...
timeout and safe nested diagnostics (#126). The older unconditional READY
wording does not describe current execution, and the integrated repairs preserve
historical Item-4/5 acceptance without authorizing anything. Attempt 3 requires
an explicit owner / preregistration-compliant ruling. Issue #101 owns measured
...
- **Now / critical path:** the D1 transport repairs (#114 → PR #120, #126 →
  PR #128) are reviewed and integrated. The remaining step is the owner /
  preregistration-compliant ruling on whether D1 research attempt 3 is
  authorized after the attempt-2 failure; only then does #101 resume with a
  fresh registered preflight and maturity observation.
...
| 6 | Development run games 1–6 | ACTIVE / D1: transport repairs integrated (PR #120, PR #128); attempts 1 and 2 are immutable and produced no corpus → owner/preregistration ruling on attempt 3 → fresh #101 preflight and maturity observation → one authorized attempt → real corpus acceptance → registered pair → D1 acceptance → D2–D6 sequentially |
