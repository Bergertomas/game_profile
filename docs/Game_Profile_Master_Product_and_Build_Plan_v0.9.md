...
Autonomous engineering does not grant autonomous editorial, data or production
**configuration** authority. On 5 September 2026 Tomas approved a bounded
standing delegation for **code deployment**: the orchestrator may independently
accept, merge and deploy in-scope reviewed engineering work through the existing
`main` → Workers wiring. Working Agreement §4.1 owns the conditions — exact-head
review, actually successful applicable CI, sound current-base integration,
pre/post canonical-origin verification and a known rollback reference — and the
preserved owner gates. A `main` merge that deploys is therefore an effect to
verify and be able to roll back, not a per-PR owner gate. Claude and other
workers remain implementers and never accept, merge or deploy their own work.
