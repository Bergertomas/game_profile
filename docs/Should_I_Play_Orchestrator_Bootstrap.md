...
- The event-driven ChatGPT Work wake bridge is **primary** (merged in PR #83, repaired in PR #98): its live smoke test in `docs/operations/ChatGPT_Work_GitHub_Wake.md` passed on 2026-09-04 and the owner promoted the event path to the primary completion signal. The hourly/scheduled autonomous checkpoint is retained as the **watchdog/recovery** path and is no longer the throughput clock. The bridge emits bounded event metadata only; it never chooses successor work, decides acceptance, merges, or advances the checklist, and the awakened orchestrator still claims an event before any mutation and fails closed. That guide's §7.1 owns the per-gate evidence and the assertions still open; that guide and Working Agreement §9.1 own the contract.
