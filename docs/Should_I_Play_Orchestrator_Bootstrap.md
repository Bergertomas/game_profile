...
- **PR #91 (`a51be6d`)** made digest-bound artifact writes verbatim, read-back verified and attempt-immutable, covering defects 1 and 2.

**Issue #87 is closed.** #87 records those as implementation corrections, not a methodology amendment; #84, #87, #90 and #91 own the exact findings, the orchestrator's holdout-isolation interpretation and the required proofs — do not restate or re-derive them here. Child issue #88 remains open on GitHub although its PR #91 merged; that is issue bookkeeping, not a surviving engineering blocker.

**The frontier is issue #95: a final read-only preflight.** It revalidates the merged A→B→C path after #90/#91, revalidates current-state Alan Wake 2 maturity eligibility under ADR 0035, and returns a READY / NOT READY engineering recommendation; it spends no billable call. Only on READY does the orchestrator's measured research execution follow, and then the paired scoring execution, under the preregistered contract. **No live D1 research or scoring has occurred.** No engineering agent executes a live D1 research or scoring call on its own initiative.
