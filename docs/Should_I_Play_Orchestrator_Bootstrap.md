...
- The scheduled watchdog that promotion retains is **armed**: the hourly, recovery-only ChatGPT task `Should I Play — Watchdog` (automation `6a9a57402f248191857fc31c2cd46baf`). It is a recovery path, not a throughput clock; the wake guide owns its contract and only Tomas changes that schedule.
...
As of 2026-09-04, against `main` at `8c54185a68297ed9f3a0c5b5ed7ffd5ae94049eb`:
...
- Item 6 — development run games D1–D6: **active**, and **no longer blocked on issue #87 or waiting on issue #95**. D1 (Alan Wake 2) is the current development run; preparation slices A, B and C are all merged, so the D1 engineering transport exists end to end. One measured D1 research attempt has since been executed and refused before scoring; no scoring execution has been performed. The engineering corrections behind the post-Slice-C readiness audit's **BLOCKED** ruling (issue #84) have landed — PR #90 and PR #91 are merged — and the final read-only maturity/readiness preflight (issue #95) returned an engineering-readiness recommendation of **READY**. Issues #84, #87, #88, #89 and #95 are all closed as completed. **Issue #101 is open, owns the detailed D1 execution record, and remains the exact resumable frontier:** its attempt 1 ran and was refused before scoring, and D1 is now blocked on an owner-reserved scope decision — see the attempt-1 paragraph below.
- Items 7–12: unchanged.
- **No Phase 3A calibration scoring has begun: D1 scoring requests remain 0, and no D1 corpus has been frozen from real evidence.** Exactly one live D1 research request has been made, and the deterministic freeze refused its output. Slices B and C merged the research-collection/corpus-freeze and paired primary/audit scoring *transports*, proved against synthetic fixtures; a merged transport is not an executed run. In the repository, only D1 preparation records and those transports exist; attempt 1's immutable capture is a git-ignored run artifact, and issue #101 carries its record. Verify against live GitHub before asserting otherwise.
...
**Engineering readiness is READY, and issue #95 is closed as completed.** That final read-only preflight re-drove the merged A→B→C transport end to end against `main` at `a5c66a2c138c897b468bc1b392fc87d38721878c` with synthetic fixtures only, re-verified current-state Alan Wake 2 maturity/bounded-change eligibility under ADR 0035 from first-party sources, spent no billable call, made no live D1 research or scoring call, and handed the exact next frontier to the orchestrator. It also discloses one **non-blocking** residual in the holdout guard's identifier patterns, with a mandatory pre-spend operator inspection and an optional bounded correction the orchestrator may take first. **#95 is the detailed evidence owner** for that finding, the exact operator command sequence and the maturity-observation shape — read it there rather than restating it. READY is an engineering recommendation, not an owner or orchestrator acceptance. Everything merged since that audited head (PR #98 and PR #105) touched only the wake bridge, its tests and operations docs; no calibration code changed.

**D1 attempt 1 is a real measured research attempt, and issue #101 owns its execution record.** Run on 2026-09-04 from an authorized non-CI runtime outside GitHub Actions, it made exactly one live registered `gpt-5.6-sol` research request; the deterministic corpus freeze then refused the returned output before any scoring, because the research model stopped on an unresolved owner scope question — whether The Final Draft / New Game Plus belongs inside D1's preregistered base-main-campaign scope. **The refused attempt is immutable evidence. No corpus has been frozen from real evidence, and D1 scoring requests remain 0.**

That scope question is owner-reserved. Watchdog recovery comment `5546141499` on issue #101 carries the decision options and the orchestrator's recommendation; they are deliberately not duplicated here. **No retry is authorized by this checkpoint.** A further measured research attempt must be explicitly authorized under the preregistration/change-control rules after Tomas's scope ruling, and no agent infers either decision, edits a controlled input to resolve it, or begins D2 work while D1 is blocked. The paired scoring execution follows **only after** a frozen real corpus exists and all preflight gates pass. No engineering agent executes a live D1 research or scoring call on its own initiative, and GPT-5.6 Sol High remains the sole D1 editorial scorer.
