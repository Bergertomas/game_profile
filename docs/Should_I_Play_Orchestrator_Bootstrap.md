...
- The event-driven ChatGPT Work wake bridge is **merged but candidate** (PR #83): the GitHub side is implemented, and it becomes the primary completion signal only after the live bot-comment smoke test in `docs/operations/ChatGPT_Work_GitHub_Wake.md` passes. Until then the hourly autonomous checkpoint remains the normal throughput clock as well as the watchdog. The bridge emits bounded event metadata only; it never chooses successor work, decides acceptance, merges, or advances the checklist. That guide and Working Agreement §9.1 own the contract.
