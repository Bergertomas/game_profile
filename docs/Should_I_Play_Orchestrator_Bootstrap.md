...
## Active checkpoint — Phase 3A, Item 6 / D1 attempt-4 authorization gate

Checkpoint baseline: 2026-09-05, `main` at `301644adee5bacd13a3f01c5ea965b583a847bf6` (main CI `33991167319` passed). Recovery is coordinated in #113; the [Recovery Execution Plan](operations/Should_I_Play_Recovery_Execution_Plan_2026-09-05.md) owns assignments and the return to the unchanged checklist. Verify live issues/PRs before acting.

**Immediate precondition:** the next measured research call is **attempt 4**, and it is **not authorized by any result recorded here**. The earlier #114 transport-v2 precondition is closed — PR #120 integrated the wrapper-owned capture hashing and scorer-visible provenance repair, and #115's deployment/recovery/dependency evidence landed in PR #117. Measured attempt 2 then ran exactly once and failed at the transport boundary; the engineering blocker that failure raised (#126) was corrected by PR #128. Attempt 3 was explicitly authorized by the owner on #101 (comment `5554711009`), executed exactly once on the credentialed local Mac, and ended `failed_api` on a provider tokens-per-minute rate limit after `334398 ms` — past the old 300-second boundary, so the #128 correction operated as intended. Neither that result nor #128 authorizes a further call: attempt 4 requires a separate explicit owner / preregistration-compliant ruling after the TPM failure is reviewed, and no agent may infer it. The earlier READY ruling was based on synthetic transport tests and remains historical evidence; it clears nothing here. The earlier hold on main integration remains lifted: under the 5 September owner-approved delegation, the orchestrator integrates in-scope reviewed engineering work under Working Agreement §4.1's conditions. #101 remains the measured-execution frontier and owns all three immutable attempt records — read it live, including any comment later than `5554753400`, before acting on attempt numbering or authorization.
...
- Item 6 — development run games D1–D6: **active**. D1 (Alan Wake 2) is the current development run; preparation slices A, B and C are merged, and the #114 transport-v2 repair is integrated by PR #120. Three measured research attempts have executed under issue #101, and all three are immutable:
  ...
  - **Attempt 4 is not authorized.** Neither attempt 3's rate-limit result nor the bounded transport correction (#126 → PR #128) supplies attempt authorization; a passing command, a green CI run and a passing dry preflight do not either. A further measured call requires a fresh explicit owner / preregistration-compliant ruling recorded in GitHub after the TPM failure is reviewed. Under the cross-tool handoff rule a material decision that exists only in chat is not governing until it is recorded, so read #101's live state — including comments later than `5554753400` — and confirm attempt numbering and authorization there rather than inferring either. See the measured-step paragraph below.

  **Issue #101 remains open and is the exact resumable frontier.**
- Items 7–12: unchanged.
- **No Phase 3A calibration scoring has begun; scoring calls remain 0.** No D1 research attempt has produced a frozen corpus or an accepted scoring input: attempt 1's refusal artifacts and the attempt-2 and attempt-3 `failed_api` records are on issue #101. No primary/audit scoring execution has run. Attempts 1, 2 and 3 remain immutable evidence and are never repaired, overwritten or fed back as model context. Verify the issue's live state before asserting a later frontier.
