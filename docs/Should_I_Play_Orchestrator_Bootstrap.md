...
As of 2026-09-04, against `main` at `e2dd2f1f6e54ed56cfacadea3a54f3bcd1530f48`:
...
- Item 6 — development run games D1–D6: **active**, and **no longer blocked on issue #87 or waiting on issue #95**. D1 (Alan Wake 2) is the current development run; preparation slices A, B and C are all merged, so the D1 engineering transport exists end to end, and the run's research/scoring executions have **not** been performed. The engineering corrections behind the post-Slice-C readiness audit's **BLOCKED** ruling (issue #84) have landed — PR #90 and PR #91 are merged — and the final read-only maturity/readiness preflight (issue #95) returned an engineering-readiness recommendation of **READY**. Issues #84, #87, #88, #89 and #95 are all closed as completed. **Issue #101 is open and is the exact resumable frontier:** the orchestrator's own measured D1 research execution under the frozen preregistered contract, which has **not** been run.
...
**The next measured step is the orchestrator's own, and issue #101 owns its execution record.** It is the measured D1 research execution under the frozen preregistered contract — `npm run calib:d1-research -- --live` on a credentialed non-CI runtime, since the harness refuses `--live` inside CI, with a maturity observation freshly stamped at the moment of observation because the gate refuses one older than 24 hours. The paired scoring execution follows **only after** that frozen real corpus exists and all preflight gates pass. **Neither call has been made: no live D1 research or scoring has occurred, and no corpus has been frozen from real evidence.** No engineering agent executes a live D1 research or scoring call on its own initiative, and GPT-5.6 Sol High remains the sole D1 editorial scorer.
