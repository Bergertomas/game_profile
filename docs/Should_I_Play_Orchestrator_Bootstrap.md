...
## Active checkpoint — Phase 3A, Item 6 / D1 transport recovery

Checkpoint baseline: 2026-09-05, `main` at `e7dd4aae3623d6cb70e51ea2b8a7d964b96f134d`. Recovery is coordinated in #113; the [Recovery Execution Plan](operations/Should_I_Play_Recovery_Execution_Plan_2026-09-05.md) owns assignments and the return to the unchanged checklist. Verify live issues/PRs before acting.

**Immediate precondition:** #114 must correct and independently prove wrapper-owned capture hashing and complete scorer-visible provenance before any new measured call. The earlier READY ruling was based on synthetic transport tests and remains historical evidence; it does not clear these new findings. #115 independently prepares deployment/recovery/dependency evidence. The earlier hold on main integration is lifted: under the 5 September owner-approved delegation, the orchestrator integrates in-scope reviewed engineering work under Working Agreement §4.1's conditions. #101 remains the measured-execution frontier.
...
- Item 6 — development run games D1–D6: **active**. D1 (Alan Wake 2) is the current development run; preparation slices A, B and C are merged; #114 addresses newly discovered engineering blockers before another measured call. Measured research attempt 1 ran once under issue #101 and was correctly refused before corpus freeze: the model withheld the source manifest/corpus because the frozen D1 wording did not explicitly classify `The Final Draft` / New Game Plus. No scoring call ran. Tomas resolved the owner scope gate on 2026-09-04: D1 is the current patched standard first-playthrough base main campaign; `The Final Draft` / New Game Plus, `Night Springs` and `The Lake House` are excluded as separate replay/expansion scopes. The clarification is integrated in PR #112; attempt 2 remains conditionally authorized after the #114 engineering precondition and fresh preflight pass. **Issue #101 remains open and is the exact resumable frontier.**
- Items 7–12: unchanged.
- **No Phase 3A calibration scoring has begun.** D1 research attempt 1 made one measured API request but produced no frozen corpus and no accepted scoring input; its refusal artifacts are recorded on issue #101. No primary/audit scoring execution has run. Attempt 1 remains immutable evidence and is never repaired or overwritten. Verify the issue's live state before asserting a later frontier.
...
The Item 5 ruling accepted the staging architecture; it did not by itself authorize the authoritative migration or the merge. Under Tomas's explicit, bounded authorization on PR #52, the orchestrator ran the read-only preflight and applied migration `0011_igdb_staging` to the authoritative production database, then verified it (one Drizzle journal row at `created_at=1788445599007`, the 11 expected `igdb_*` tables, the identity/provider uniqueness indexes, the append-only trigger). PR #52 was reconciled against `main` at head `d3b49e9eecafb3341fa2eaf9519beb452b314bd3` and merged through the ordinary reviewed path. The old Item-5 integration blocker is therefore closed. The new #114 transport precondition remains separate.
...
**Historical readiness: #95 ruled READY and is closed as completed. Current execution additionally requires #114.** That final read-only preflight re-drove the merged A→B→C transport end to end against `main` at `a5c66a2c138c897b468bc1b392fc87d38721878c` with synthetic fixtures only, re-verified current-state Alan Wake 2 maturity/bounded-change eligibility under ADR 0035 from first-party sources, spent no billable call, made no live D1 research or scoring call, and handed the exact next frontier to the orchestrator. It also discloses one **non-blocking** residual in the holdout guard's identifier patterns, with a mandatory pre-spend operator inspection and an optional bounded correction the orchestrator may take first. **#95 is the detailed evidence owner** for that finding, the exact operator command sequence and the maturity-observation shape — read it there rather than restating it. READY is an engineering recommendation, not an owner or orchestrator acceptance. That ruling preceded the later PR #112 D1 scope clarification and the #113 audit recovery findings; do not extrapolate its synthetic proof to current live readiness.

**The next measured step belongs to the authorized registered runtime, and issue #101 owns its execution record.** Only after #114 is accepted and safely integrated, the already-integrated PR #112 scope clarification is verified, and fresh preflight passes, run `npm run calib:d1-research -- --live --maturity <observation.json> --attempt 2` on a credentialed non-CI runtime with a freshly stamped maturity observation. Attempt 1 is retained refusal evidence and must not be overwritten or fed back as model context. Independently audit a successful frozen corpus before any paired scoring call. The registered paired scoring execution follows **only after** that real corpus exists and all preflight gates pass. No primary/audit scoring has occurred. No engineering agent substitutes editorial judgment, and GPT-5.6 Sol High remains the sole D1 editorial scorer.
...
- Master Plan v0.9 now records Item 6 active with the #114 transport precondition and the original Items 1–12 mapping in Appendix B. The Plan remains the product/roadmap constitution; this bootstrap points to the live frontier.
