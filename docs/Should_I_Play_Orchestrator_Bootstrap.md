...
As of 2026-09-04, against `main` at `3acca852e387b6c2256156a9eb15884e68d70f14`:
...
- Item 6 — development run games D1–D6: **active**. D1 (Alan Wake 2) is the current development run; preparation slices A, B and C are all merged, so the D1 engineering transport exists end to end, and the run's research/scoring executions have **not** been performed. Measured execution is **blocked**: the post-Slice-C readiness audit (issue #84) returned a **BLOCKED** ruling, and issue #87 carries the bounded engineering corrections that must land and be accepted first.
...
The A/B/C transports are merged, but **D1 measured execution is blocked, and the frontier is a correction rather than a new build slice.** The read-only post-Slice-C execution readiness audit (issue #84) delivered its ruling against this same `main` and it is **BLOCKED**. Issue #87 is the active engineering blocker: bounded implementation corrections covering digest-bound artifact persistence, retry/attempt preservation, and the overbroad corpus holdout-title guard, with their regression proofs. #87 records those as implementation corrections, not a methodology amendment; the audit issue and #87 own the exact findings, the orchestrator's holdout-isolation interpretation and the required proofs — do not restate or re-derive them here.

Until #87's corrections land and are accepted, no measured D1 pair may be spent. What then remains is the orchestrator's measured research and scoring executions under the preregistered contract. **No live D1 research or scoring has occurred.** No engineering agent executes a live D1 research or scoring call on its own initiative.
...
- `docs/calibration/Phase_3A_D1_Paired_Scoring_Transport_Handoff.md` — the slice-C gates, pair proof, validation boundary and operator sequence; mandatory before any D1 scoring execution or readiness ruling; landed with PR #81
