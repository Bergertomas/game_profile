...
## Active checkpoint — Phase 3A, Item 6 / D1 attempt-3 authorization gate

Checkpoint baseline: 2026-09-05, `main` at `3cda870849e648727dc793f6d448563cf4b8e5bb`. Recovery is coordinated in #113; the [Recovery Execution Plan](operations/Should_I_Play_Recovery_Execution_Plan_2026-09-05.md) owns assignments and the return to the unchanged checklist. Verify live issues/PRs before acting.

**Immediate precondition:** the next measured research call is **attempt 3**, and it is **not authorized**. The earlier #114 transport-v2 precondition is closed — PR #120 integrated the wrapper-owned capture hashing and scorer-visible provenance repair, and #115's deployment/recovery/dependency evidence landed in PR #117. Measured attempt 2 then ran exactly once and failed at the transport boundary; the engineering blocker that failure raised (#126) was corrected by PR #128, which is merged as this checkpoint's baseline. Neither #126 nor #128 authorizes a further call: attempt 3 requires a separate explicit owner / preregistration-compliant ruling after the attempt-2 failure is reviewed, and no agent may infer it. The earlier READY ruling was based on synthetic transport tests and remains historical evidence; it clears nothing here. The earlier hold on main integration remains lifted: under the 5 September owner-approved delegation, the orchestrator integrates in-scope reviewed engineering work under Working Agreement §4.1's conditions. #101 remains the measured-execution frontier and owns both immutable attempt records.
...
- Item 6 — development run games D1–D6: **active**. D1 (Alan Wake 2) is the current development run; preparation slices A, B and C are merged, and the #114 transport-v2 repair is integrated by PR #120. Two measured research attempts have executed under issue #101, and both are immutable:
  ...
  - **Attempt 3 has not executed and is not authorized.** A local runtime posted an attempt-3 claim on #101 and released it without execution: measured calls 0, and no request, spend, response, ledger row, run directory, corpus or scoring artifact was created. The bounded transport correction (#126 → PR #128) supplies engineering readiness only, never attempt authorization. #101 also carries a claim that the owner authorized one retry in chat; under the cross-tool handoff rule a material decision that exists only in chat is not yet governing, so treat attempt 3 as gated until an explicit owner / preregistration-compliant ruling is recorded in GitHub. See the measured-step paragraph below.

  **Issue #101 remains open and is the exact resumable frontier.**
- Items 7–12: unchanged.
- **No Phase 3A calibration scoring has begun; scoring calls remain 0.** Neither D1 research attempt produced a frozen corpus or an accepted scoring input: attempt 1's refusal artifacts and attempt 2's `failed_api` record are on issue #101. No primary/audit scoring execution has run. Attempts 1 and 2 remain immutable evidence and are never repaired, overwritten or fed back as model context. Verify the issue's live state before asserting a later frontier.
