...
- Item 6 — development run games D1–D6: **active**. D1 (Alan Wake 2) is the current development run; preparation slices A, B and C are all merged, so the D1 engineering transport is complete end to end, and the run's research/scoring executions have **not** been performed.
...
The engineering dependency frontier is therefore no longer a build slice. What remains before a measured D1 pair is spent: the read-only post-Slice-C execution readiness audit (issue #84, open), and then the measured research and scoring executions, which are the orchestrator's to run under the preregistered contract. No engineering agent executes a live D1 research or scoring call on its own initiative.
