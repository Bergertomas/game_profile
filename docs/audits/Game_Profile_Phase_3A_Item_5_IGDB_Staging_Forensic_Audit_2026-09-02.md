...
| Enum→table ids (live proof, 2026-09-03) | The migration preserves historical enum numbering as table ids. Documented zero values: `game_type` `main_game = 0`, `game_status` `released = 0`, `date_format` `YYYYMMMMDD = 0`. Enums numbered from 1 include `region` (`europe = 1`) and external-game `category` (`steam = 1`). | Those three references accept nonnegative ids; every other reference and every record id stays strictly positive. Negative ids refused everywhere. |
| Dump CSV shape (live proof, 2026-09-03) | The real `platforms` dump (43,711 bytes, 17 columns, `LONG[]` + `TIMESTAMP`) contains a quoted value spanning a physical newline. The docs do not state the CSV dialect. | The dump reader is record-aware: a newline ends a record only outside quotes; doubled quotes, embedded commas and CRLF/LF/CR handled; unclosed quoting still fails closed. |
