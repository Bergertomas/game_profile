...
| `lib/igdb/record.ts` | The one intermediate `IgdbGameRecord`; API JSON parser with expanded/bare reference handling; per-field id contracts (zero-valued enum refs vs strictly positive entity refs); unexpanded-field reporting | ADR 0037 §4 |
| `lib/igdb/dump.ts` | Data Partner listing/descriptor schemas; **record-aware** CSV reader (quoted multiline values, doubled quotes, embedded commas, CRLF/LF/CR; fail-closed on unclosed quoting); cells parsed by declared schema type; per-endpoint tables assembled into records | ADR 0037 §4 |
...
**Two provider-contract facts the first credentialed run established (2026-09-03).** Proof A passed unchanged. Proofs B and C failed closed, and both failures were ours, not IGDB's:

1. **Zero is a legitimate id in three enum→table contracts.** The probe returned `game_type.id = 0` and six `release_dates[].date_format.id = 0`; the parser required every id to be positive and refused the record. The enum→table migration preserves the historical enum numbering, and three of those enums are documented as starting at zero — `game_type` `main_game = 0`, `game_status` `released = 0`, `date_format` `YYYYMMMMDD = 0` (Game Enums and Release Date Enums, read 2026-09-03). `IGDB_ZERO_VALUED_ENUM_REFERENCES` names exactly those three and `namedRef` takes the id contract per field, so they accept nonnegative ids while everything in `IGDB_POSITIVE_ONLY_REFERENCES` — `release_region` (`europe = 1`), `external_game_source` (`steam = 1`), the tables with no legacy enum, the ordinary entities `platform` and `company`, and every record id — still refuses zero. A negative id is refused everywhere.

2. **Real dump CSV carries quoted values that cross a newline.** The `platforms` descriptor and download succeeded (43,711 bytes, 17 columns, `LONG[]` + `TIMESTAMP`), but `parseDumpCsv` refused the file with "Unterminated quoted field in CSV line": a quoted `summary` spans physical lines and the line-oriented reader tore the record in half. `parseCsvRecords` now reads RECORDS — a newline ends a record only outside quotes — and handles doubled quotes, embedded commas and CRLF/LF/CR separators, while still failing closed on quoting it cannot close. `observeDumpEncodings` reads the same records, so a declared array or timestamp cell is still taken from the right column of the right row and a `{1,2}` or ISO-looking substring inside a multiline summary proves nothing.
