...
**Workflow files cannot be pushed by the runner.** A push containing a `.github/workflows/**` change is rejected outright — `refusing to allow a GitHub App to create or update workflow ... without 'workflows' permission` — which is why every workflow change in PR #83 and issue #94 was committed by Tomas. This is a transport limitation, not a review step: the runner may still author, verify and hand over the exact file. When a runner task needs to change a workflow, it should stage the complete file somewhere it can write (`docs/operations/patches/<name>.yml` is the convention), state plainly in its handoff that the change is not live, and keep any test reading the real workflow path so the staged copy cannot silently become a second source of truth. The owner applies it with one `git mv`, and the staged copy is deleted in that same change.
