...
/**
 * The share card for one Game Profile.
 *
 * Prerendered at build time for every published profile, so it ships as a
 * static asset and costs nothing at request time on Cloudflare Workers.
 *
 ...
 *
 * It takes a `ProfileView`, so the primary route and the sibling-scope route
 * share one implementation and a sibling card cannot drift into showing the
 * primary scope's numbers.
 */

export const SHARE_CARD_ALT = "Game Profile share card";
export const SHARE_CARD_SIZE = { width: 1200, height: 630 };
export const SHARE_CARD_CONTENT_TYPE = "image/png";

export function shareCard(profile: ProfileView): ImageResponse {
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          {/* Satori's own element, not the DOM's: next/image has no meaning
              inside an ImageResponse, which renders to a PNG rather than to a
              document. The rule fires because this now lives outside app/. */}
