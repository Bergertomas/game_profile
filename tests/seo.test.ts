...
describe("sitemap.xml", () => {
  it("lists the home page, the methodology and every published game", async () => {
    ...
    // Home, methodology, and one entry per profile — the count is asserted so
    // a new indexable route cannot appear without a decision. /about is
    // deliberately absent: it is not published until it has a corrections
    // route, and an unpublished page must not be advertised to a crawler.
